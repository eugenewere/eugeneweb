...
def messages_chart(request):
    ...
    for months_choice in months_choices_int:
        month_data.append(
            ContactUs.objects.filter(created_at__month=months_choice).count())
    return JsonResponse({
        'monthlabels': months_choices,
        'monthdata': month_data,
    })


def project_pie_chart(request):
    categories = []
    projects = []
...
def project_line_chart(request):
    import datetime
    month_data = []
    months_choices = []
    months_choices_int = []
    ...
    for months_choice in months_choices_int:
        month_data.append(Portfolio.objects.filter(created_at__month=months_choice).count())
